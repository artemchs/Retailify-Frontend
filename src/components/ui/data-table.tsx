...
import { BatchEditProductsDialogProps } from '@/features/products/components/actions/batch-edit/BatchEditProductsDialog'
...
interface DataTableProps<TData, TValue> {
    ...
    batchEditComponent?: React.FC<BatchEditProductsDialogProps>
}
...
export function DataTable<TData, TValue>({
    ...
    batchEditComponent: BatchEditComponent,
            ...
            const handleSelectionState = (selections?: RowSelectionState) => {
                const items = selections
                    ? Object.keys(selections).map(
                          (key) =>
                              table.getSelectedRowModel().rowsById[key]
                                  ?.original ||
                              selectedRowsWithData?.find(
                                  (row) => row.id === key
                              )
                      )
                    : []
                            ...
                            <TableRow>
                                ...
                                >
                                    <p className='text-destructive'>
                                        Возникла неожиданная ошибка во время
                                        загрузки. Попробуйте перезагрузить
                                        страницу. Если проблема не пропадает,
